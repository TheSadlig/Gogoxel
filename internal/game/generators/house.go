...
// NewHouseGenerator returns a constructive-solid-geometry house generator.
// The scale defines the full height of the house, including the roof ridge.
func NewHouseGenerator(scale uint32) *houseGenerator {
	if scale == 0 {
		scale = defaultHouseScale
	}

	return &houseGenerator{scale: scale}
}
...
func (g *houseGenerator) Generate(width, height, depth uint32) ([]uint8, [255]uint32) {
	total := int(width * height * depth)
	data := make([]uint8, total)
	if width == 0 || height == 0 || depth == 0 {
		return data, housePalette
	}

	layout := g.layout(int(width), int(height), int(depth))
	if !layout.valid {
		return data, housePalette
	}
	...
	fillSolid(data, width, height, layout.shellBounds, shell, houseWallVoxel)

	for _, piece := range layout.foundationPieces {
		fillSolid(data, width, height, piece, box(piece), houseFoundationVoxel)
	}
	for _, partition := range layout.partitions {
		fillSolid(data, width, height, partition, box(partition), houseWallVoxel)
	}
	for _, roof := range layout.roofs {
		fillSolid(data, width, height, roof.bounds, gableRoof(roof.bounds, roof.axis), houseRoofVoxel)
	}
	for _, chimney := range layout.chimneys {
		fillSolid(data, width, height, chimney, box(chimney), houseWallVoxel)
	}
...
func fillSolid(data []uint8, width, height uint32, bounds solidBounds, solid solidFunc, voxel uint8) {
	if !bounds.valid() {
		return
	}
