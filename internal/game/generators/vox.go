...
// const defaultModelPath = "third_party/voxel-model/vox/monument/monu1.vox"
const defaultModelPath = "third_party/voxel-model/svo/buddha_16k.rsvo"

const (
	rsvoFilledVoxel   uint8 = 1
	rsvoRankBlockSize int   = 1024
)
...
// NewGeneratorFromFile loads a supported voxel model file into a Generator.
func NewGeneratorFromFile(path string) (Generator, error) {
	...
	return NewGeneratorFromBytes(data)
}

// NewGeneratorFromBytes loads a supported voxel model payload into a Generator.
func NewGeneratorFromBytes(data []byte) (Generator, error) {
	...
	case "VOX ":
		return NewVOXGeneratorFromBytes(data)
	case "RSVO":
		return NewRSVOGeneratorFromBytes(data)
	default:
		return nil, fmt.Errorf("unsupported model format %q", string(data[:4]))
	}
}

// NewVOXGeneratorFromFile loads a MagicaVoxel .vox file into a Generator.
...
// NewVOXGeneratorFromBytes loads a MagicaVoxel .vox payload into a Generator.
func NewVOXGeneratorFromBytes(data []byte) (*voxGenerator, error) {
	...
	return &voxGenerator{model: model}, nil
}

// NewRSVOGeneratorFromFile loads an .rsvo sparse voxel octree into a Generator.
...
// NewRSVOGeneratorFromBytes loads an .rsvo payload into a Generator.
func NewRSVOGeneratorFromBytes(data []byte) (*rsvoGenerator, error) {
	...
	return &rsvoGenerator{model: model}, nil
}

func (g *voxGenerator) Generate(width, height, depth uint32) ([]uint8, [255]uint32) {
	total := int(width * height * depth)
	data := make([]uint8, total)
	if width == 0 || height == 0 || depth == 0 {
		return data, g.model.palette
	}
	if len(g.model.voxels) == 0 || g.model.sizeX == 0 || g.model.sizeY == 0 || g.model.sizeZ == 0 {
		return data, g.model.palette
	}

	scale := minFloat64(
		float64(width)/float64(g.model.sizeX),
		float64(height)/float64(g.model.sizeY),
		float64(depth)/float64(g.model.sizeZ),
	)
	if scale <= 0 {
		return data, g.model.palette
	}

	scaledWidth := minInt(int(width), maxInt(1, int(math.Ceil(float64(g.model.sizeX)*scale))))
	scaledHeight := minInt(int(height), maxInt(1, int(math.Ceil(float64(g.model.sizeY)*scale))))
	groundedDepth := minInt(int(depth), maxInt(1, int(math.Ceil(float64(g.model.sizeZ)*scale))))

	offsetX := maxInt(0, (int(width)-scaledWidth)/2)
	offsetY := maxInt(0, (int(height)-scaledHeight)/2)
	offsetZ := 0

	layerStride := int(width * height)
	for _, voxel := range g.model.voxels {
		x0 := offsetX + int(math.Floor(float64(voxel.x)*scale))
		x1 := offsetX + int(math.Ceil(float64(voxel.x+1)*scale)) - 1
		y0 := offsetY + int(math.Floor(float64(voxel.y)*scale))
		y1 := offsetY + int(math.Ceil(float64(voxel.y+1)*scale)) - 1
		z0 := offsetZ + int(math.Floor(float64(voxel.z)*scale))
		z1 := offsetZ + int(math.Ceil(float64(voxel.z+1)*scale)) - 1

		x0 = clampInt(x0, 0, int(width)-1)
		x1 = clampInt(x1, 0, int(width)-1)
		y0 = clampInt(y0, 0, int(height)-1)
		y1 = clampInt(y1, 0, int(height)-1)
		z0 = clampInt(z0, 0, groundedDepth-1)
		z1 = clampInt(z1, 0, groundedDepth-1)
		if x1 < x0 || y1 < y0 || z1 < z0 {
			continue
		}

		for z := z0; z <= z1; z++ {
			zOffset := z * layerStride
			for y := y0; y <= y1; y++ {
				rowOffset := zOffset + y*int(width)
				for x := x0; x <= x1; x++ {
					data[rowOffset+x] = voxel.color
				}
			}
		}
	}
...
func (g *rsvoGenerator) Generate(width, height, depth uint32) ([]uint8, [255]uint32) {
	total := int(width * height * depth)
	data := make([]uint8, total)
	if width == 0 || height == 0 || depth == 0 {
		return data, g.model.palette
	}
	if g.model.topLevel < 0 || !g.model.bounds.valid() {
		return data, g.model.palette
	}

	sourceWidth := g.model.bounds.maxX - g.model.bounds.minX + 1
	sourceHeight := g.model.bounds.maxY - g.model.bounds.minY + 1
	sourceDepth := g.model.bounds.maxZ - g.model.bounds.minZ + 1
	...
	scale := minFloat64(
		...
		float64(depth)/float64(sourceDepth),
	)
	if scale <= 0 {
		return data, g.model.palette
	}

	scaledWidth := minInt(int(width), maxInt(1, int(math.Ceil(float64(sourceWidth)*scale))))
	scaledHeight := minInt(int(height), maxInt(1, int(math.Ceil(float64(sourceHeight)*scale))))
	offsetX := maxInt(0, (int(width)-scaledWidth)/2)
	offsetY := maxInt(0, (int(height)-scaledHeight)/2)

	rootSize := 1 << uint(g.model.topLevel)
	root := rsvoNode{level: g.model.topLevel, nodeIndex: 0, minX: 0, minY: 0, minZ: 0, size: rootSize}
	g.fillNode(data, width, height, depth, root, scale, offsetX, offsetY)
...
func parseRSVO(data []byte) (rsvoModel, error) {
	...
	model := rsvoModel{topLevel: topLevel, levels: levels, palette: rsvoPalette}
...
func (g *rsvoGenerator) fillNode(data []uint8, width, height, depth uint32, node rsvoNode, scale float64, offsetX, offsetY int) {
	projected, ok := g.projectNodeBounds(node, scale, int(width), int(height), int(depth), offsetX, offsetY)
	if !ok {
		return
	}
	if node.level == 0 || (projected.minX == projected.maxX && projected.minY == projected.maxY && projected.minZ == projected.maxZ) {
		fillSolid(data, width, height, projected, box(projected), rsvoFilledVoxel)
		return
	}

	mask := g.model.levels[node.level].masks[node.nodeIndex]
	if mask == 0 {
		return
	}
	childBase := g.model.childBase(node.level, node.nodeIndex)
	...
	for bitIndex := 0; bitIndex < 8; bitIndex++ {
		...
		childOffset++
		g.fillNode(data, width, height, depth, child, scale, offsetX, offsetY)
	}
}

func (g *rsvoGenerator) projectNodeBounds(node rsvoNode, scale float64, width, height, depth, offsetX, offsetY int) (solidBounds, bool) {
	sourceMinX := maxInt(node.minX, g.model.bounds.minX)
	sourceMaxX := minInt(node.minX+node.size-1, g.model.bounds.maxX)
	sourceMinY := maxInt(node.minY, g.model.bounds.minY)
	sourceMaxY := minInt(node.minY+node.size-1, g.model.bounds.maxY)
	sourceMinZ := maxInt(node.minZ, g.model.bounds.minZ)
	sourceMaxZ := minInt(node.minZ+node.size-1, g.model.bounds.maxZ)
	if sourceMinX > sourceMaxX || sourceMinY > sourceMaxY || sourceMinZ > sourceMaxZ {
		return solidBounds{}, false
	}

	flippedMinZ := g.model.bounds.maxZ - sourceMaxZ
	flippedMaxZ := g.model.bounds.maxZ - sourceMinZ

	bounds := solidBounds{
		minX: offsetX + int(math.Floor(float64(sourceMinX-g.model.bounds.minX)*scale)),
		maxX: offsetX + int(math.Ceil(float64(sourceMaxX-g.model.bounds.minX+1)*scale)) - 1,
		minY: offsetY + int(math.Floor(float64(sourceMinY-g.model.bounds.minY)*scale)),
		maxY: offsetY + int(math.Ceil(float64(sourceMaxY-g.model.bounds.minY+1)*scale)) - 1,
		minZ: int(math.Floor(float64(flippedMinZ) * scale)),
		maxZ: int(math.Ceil(float64(flippedMaxZ+1)*scale)) - 1,
	}
	bounds.minX = clampInt(bounds.minX, 0, width-1)
	bounds.maxX = clampInt(bounds.maxX, 0, width-1)
	bounds.minY = clampInt(bounds.minY, 0, height-1)
	bounds.maxY = clampInt(bounds.maxY, 0, height-1)
	bounds.minZ = clampInt(bounds.minZ, 0, depth-1)
	bounds.maxZ = clampInt(bounds.maxZ, 0, depth-1)
	if !bounds.valid() {
		return solidBounds{}, false
	}
