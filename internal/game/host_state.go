...
func (s *sessionState) metricsSnapshot() session.MetricsSnapshot {
	...
	return session.MetricsSnapshot{
		...
		TransferQueueUploadsWindowTotal: s.game.TransferQueueUploadsWindowTotal(),
		TransferUploadPath:              s.game.TransferUploadPath(),
		ChunkStorageStrategy:            s.game.ChunkStorageStrategy(),
		TraversalAlgorithm:              s.game.TraversalAlgorithm(),
	}
}
